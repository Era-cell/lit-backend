...
describe("Single Oracle Bandit Recommendation", () => {
    ...
    describe("should always pick the highest scored action with temperature=0", () => {
        it("should always pick the highest scored action with temperature=0", () => {
            const bandit = new SimpleBandit_1.SimpleBandit({
                oracle: new SimpleOracle_1.SimpleOracle({ weights: { action2: 1 } }),
                actions: ["action1", "action2"],
                temperature: 0,
            });
            for (let i = 0; i < 10; i++) {
    ...
    describe("recommend", () => {
        ...
        describe("recommendationId should match in trainingData", () => {
            it("should have a trainingData entry with the same recommendationId", () => __awaiter(void 0, void 0, void 0, function* () {
                const trainingData = yield bandit.accept(recommendation);
                expect(trainingData.filter((data) => data.recommendationId === recommendation.recommendationId).length).toEqual(1);
            }));
        });
    });
});
